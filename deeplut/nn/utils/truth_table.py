...
def reduce_truth_table(k: int, table: torch.Tensor, device: str) -> torch.Tensor:
    ...
    Args:
        k ([type]): truth table power
        table ([type]): Nd array where truth table represented in last 2d
        device ([type]): target device of the results

    Returns:
        [type]: Nd torch tensor -dims count not changing- reduced number of rows in the table by k
    """
