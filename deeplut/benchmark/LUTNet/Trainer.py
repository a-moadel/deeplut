...
def LUTNetTrainer(phase_name, dataset, model_warpper, n_epochs, lr, binary_optim, binarization_level, input_expanded, device):
    ...
    for epoch in range(n_epochs):
        ...
        print(':: Epoch: {} train avg loss: {:.4f} time = {}, test average loss: {:.4f}, acc: ({:.4f}%) time = {}\n'.format(
            epoch, training_loss, training_time, testing_loss, accuracy, test_time))


def create_phase(phase_name, n_epochs, lr, binary_optim, binarization_level, input_expanded):
    phase = type('', (), {})()
...
def multi_phase_training(phases, dataset, model_warpper, device):
    for phase in phases:
        ...
        LUTNetTrainer(phase.phase_name, dataset, model_warpper, phase.n_epochs, phase.lr,
                      phase.binary_optim, phase.binarization_level, phase.input_expanded, device)
