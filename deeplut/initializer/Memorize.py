...
class Memorize(BaseInitializer):
    ...
    def _input_to_id(self, inputs: np.array) -> torch.Tensor:
        ...
        _inputs[_inputs == -1] = 0
        power_arr = 2 ** torch.arange(_inputs.shape[-1]).flip(0).view(
            -1, 1
        ).to(self.device)
        return (
            (power_arr.T * _inputs)
            .sum(1)
            .view(inputs.shape[0], -1)
            .to(self.device)
        )

    def clear(self) -> None:
        self.counter = torch.zeros(
            self.table_count, self.kk, device=self.device
        ).int()

    def _update_counter(
        self, input_ids: torch.Tensor, target: torch.Tensor
    ) -> None:
        ...
        self.counter[
            table_ids.long(), input_ids.long().flatten()
        ] += _target_value
